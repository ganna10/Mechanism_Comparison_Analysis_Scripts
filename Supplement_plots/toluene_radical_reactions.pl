#!/usr/bin/perl
# radicals family budget reactions due to toluene as stacked bar plots
...
($production_rates{'radicals_mozart'}, $consumption_rates{'radicals_mozart'}) = get_rates($parents[0], 'radicals_mozart', $mozart_kpp, $mozart_mecca);
...
($production_rates{'radicals_radm2'}, $consumption_rates{'radicals_radm2'}) = get_rates($parents[1], 'radicals_radm2', $radm2_kpp, $radm2_mecca);
...
($production_rates{'radicals_racm'}, $consumption_rates{'radicals_racm'}) = get_rates($parents[1], 'radicals_racm', $racm_kpp, $racm_mecca);
...
($production_rates{'radicals_racm2'}, $consumption_rates{'radicals_racm2'}) = get_rates($parents[1], 'radicals_racm2', $racm2_kpp, $racm2_mecca);
...
sub budget_plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, plot.title, plot.legend) {data = ddply(data, .(time), colwise(sum)) ;
                                                                    ...
                                                                    plot = plot + scale_y_continuous(limits = c(-60, 100), breaks = seq(-60, 100, 20)) ;
    ...
    $R->run(q` my.colours = c("Production Others" = "#696537", "C2O3 + NO" = "#8b1537", "HCHO + hv" = "#2c9daf", "KET + hv" = "#f9c500", "MGLY + hv" = "#f3aa7f", "MGLYOX + hv" = "#f3aa7f", "CARB6 + hv" = "#f3aa7f", "CH3COCHO + hv" = "#f3aa7f", "C5COO2NO2 + OH" = "#8fd5d3", "CARB3 + hv" = "#76afca", "DCB + hv" = "#c65d6c", "NO + TCO3" = "#9bb08f", "NO + XO2" = "#0e5c28", "CSL + NO3" = "#be234b", "CRES + NO3" = "#be234b", "NO2 + PHO" = "#cc6329", "DCB1 + O3" = "#1c3e3d", "EPX + O3" = "#4b9483", "DCB2 + O3" = "#b569b3", "ONIT" = "#a67c52", "BIGALD + hv" = "#8b1537", "OH + OPEN" = "#1b695b", "OPEN + hv" = "#f9c500", "CRES + OH" = "#898989", "CRO + NO2" = "#8c6238", "UDCARB11 + hv" = "#c65d6c", "Consumption Others" = "#ee6738") `);

     $R->run(q` CairoPDF(file = "toluene_radicals_budget_reactions.pdf", width = 210, height = 200) `, 
