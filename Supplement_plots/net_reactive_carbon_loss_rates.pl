...
foreach my $mechanism (@mechanisms) {
    ...
    my @VOCs = qw(Propane Octane Propene);
...
$R->run(q` plot.data$VOC = factor(plot.data$VOC, levels = c("Propane", "Octane", "Propene")) `);
    ...
    foreach my $reaction (keys %carbon_loss_rate) {
        if ($VOC =~ /HC3/) {
            if ($mechanism eq "RACM2") {
                $carbon_loss_rate{$reaction} = $carbon_loss_rate{$reaction} * 3 / 3.6;
            } elsif ($mechanism =~ /RA/) {
                $carbon_loss_rate{$reaction} = $carbon_loss_rate{$reaction} * 3 / 2.9;
            }
        } elsif ($VOC =~ /HC8|BIGALK/) {
            if ($mechanism =~ /RA/) {
                $carbon_loss_rate{$reaction} = $carbon_loss_rate{$reaction} * 8 / 7.9;
            } elsif ($mechanism =~ /MOZ/) {
                $carbon_loss_rate{$reaction} = $carbon_loss_rate{$reaction} * 8 / 5;
            }
        } else { #propene
            if ($mechanism =~ /RA/) {
                $carbon_loss_rate{$reaction} = $carbon_loss_rate{$reaction} * 3 / 3.8;
            }
        }
    }
...
sub get_total_C {
    ...
    my @inorganic = qw( hv OH HO2 O3 NO NO2 NO3 H2O HNO3 H2 PAROP O CO2 XO2 XO2N OHOP H2O2 );
