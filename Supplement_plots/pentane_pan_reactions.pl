#!/usr/bin/perl
# PAN family budget reactions due to pentane as stacked bar plots
# Version 0 : Jane Coates 27/7/2014
...
($production_rates{'PAN_mozart'}, $consumption_rates{'PAN_mozart'}) = get_rates($parents[1], 'PAN_mozart', $mozart_kpp, $mozart_mecca);
...
($production_rates{'PAN_radm2'}, $consumption_rates{'PAN_radm2'}) = get_rates($parents[2], 'PAN_radm2', $radm2_kpp, $radm2_mecca);
...
($production_rates{'PAN_racm'}, $consumption_rates{'PAN_racm'}) = get_rates($parents[2], 'PAN_racm', $racm_kpp, $racm_mecca);
...
($production_rates{'PAN_racm2'}, $consumption_rates{'PAN_racm2'}) = get_rates($parents[2], 'PAN_racm2', $racm2_kpp, $racm2_mecca);
...
sub budget_plot { #create dataframe and then create plot
    ...
    $R->run(q` my.colours = c("Production Others" = "#696537", "CO2C3CHO + OH" = "#f8c56c", "CH3COCH2O" = "#898989", "CH3CHO + OH" = "#0352cb", "ALD + OH" = "#0352cb", "ACD + OH" = "#0352cb", "ALD2 + OH" = "#0352cb", "CH3CO3 + NO" = "#8b1537", "ACO3 + NO" = "#8b1537", "C2O3 + NO" = "#8b1537", "CO2C3CO3 + NO" = "#9bb08f", "NO + RN11O2" = "#8fd5d3", "MEKO2 + NO" = "#c9a415", "MGLY + OH" = "#f3aa7f", "ALDX + OH" = "#b569b3", "CXO3 + NO" = "#84b84c", "Consumption Others" = "#ee6738") `);

     $R->run(q` CairoPDF(file = "pentane_pan_budget_reactions.pdf", width = 210, height = 200) `, 
