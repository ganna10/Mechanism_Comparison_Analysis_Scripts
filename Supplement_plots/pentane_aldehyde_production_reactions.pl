...
$R->run(q` plot = ggplot(data, aes(x = Time, y = Rate, fill = Reaction)) `,
        ...
        q` plot = plot + theme(legend.position = c(0.95, 0.14)) `,
        q` plot = plot + theme(legend.justification = c(0.95, 0.14)) `,
        q` plot = plot + scale_fill_manual(values = my.colours, limits = rev(levels(data$Reaction))) `,
);

$R->run(q` CairoPDF(file = "pentane_aldehyde_production_budget.pdf", width = 141, height = 200) `,
        q` print(plot) `,
        q` dev.off() `,
);
