#! /usr/bin/perl
# Analyse day-time pan budget during Pentane breakdown, compare for all mechanisms - stacked bar plot
...
($production_rates{'PAN_mozart'}, $consumption_rates{'PAN_mozart'}) = get_rates($species[1], 'PAN_mozart', $mozart_kpp, $mozart_mecca, \@mozart_radicals, \@mozart_pans);
...
($production_rates{'PAN_radm2'}, $consumption_rates{'PAN_radm2'}) = get_rates($species[2], 'PAN_radm2', $radm2_kpp, $radm2_mecca, \@radm2_radicals, \@radm2_pans);
...
($production_rates{'PAN_racm'}, $consumption_rates{'PAN_racm'}) = get_rates($species[2], 'PAN_racm', $racm_kpp, $racm_mecca, \@racm_radicals, \@racm_pans);
...
($production_rates{'PAN_racm2'}, $consumption_rates{'PAN_racm2'}) = get_rates($species[2], 'PAN_racm2', $racm2_kpp, $racm2_mecca, \@racm2_radicals, \@racm2_pans);
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, plot.title, plot.legend) {data = ddply(data, .(time), colwise(sum)) ;
                                                                    ...
                                                                    plot = plot + scale_fill_manual( limits = my.legend, values = my.colours, guide = guide_legend(label.position = "bottom", label.theme = element_text(size = 160, angle = 20, face = "bold"), label.hjust = 0.5, label.vjust = 0.5)); 
                                                                    plot = plot + theme_bw() ; 
                                                                    plot = plot + theme(axis.text.x = element_text(size = 170)) ; 
                                                                    plot = plot + theme(axis.text.y = element_text(size = 170)) ; 
                                                                    ...
                                                                    plot = plot + scale_y_continuous(limits = c(-50, 70), breaks = seq(-50, 70, 20)) ;
                                                                    plot = plot + theme(panel.grid.minor.y = element_blank(), panel.grid.major.y = element_blank()) ;
                                                                    plot = plot + theme(legend.position = "bottom", legend.title = element_blank(), legend.key.size = unit(10, "cm"), legend.key = element_blank()); 
                                                                    return(plot) } `);
    ...
    $R->run(q` cb05.plot = mech.plot(cb05.data, cb05.plot.title, cb05.legend) `);
     ...
     $R->run(q` CairoPDF(file = "pentane_pan_budgets.pdf", width = 210, height = 210) `, 
