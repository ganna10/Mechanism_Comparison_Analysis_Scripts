#! /usr/bin/perl
# print out cumulative TOPP values on one plot
...
sub get_plot {
    ...
    $R->run(q` plot.lines = function () { list( geom_line(size = 3), 
                                                ...
                                                facet_grid(. ~ NMVOC, space = "free", labeller = facet_labeller), 
                                                ...
                                                theme(strip.text.x = element_text(size = 37, face = "bold", vjust = 1)), 
    ...
    my $plot = $R->run( q` plot1 = ggplot(data = subset(data, NMVOC == "C2H6" | NMVOC == "C3H8" | NMVOC == "NC4H10" | NMVOC == "IC4H10" | NMVOC == "NC5H12"), aes(x = times, y = TOPP.value, colour = Mechanism)) `,
                        ...
                        q` main.plot = grid.arrange(y.label, arrangeGrob(plot1 + theme(legend.position="none", axis.text.x = element_blank(), axis.ticks.x = element_blank()), plot2 + theme(legend.position="none", axis.text.x = element_blank(), axis.ticks.x = element_blank()), plot3 + theme(legend.position="none", axis.text.x = element_blank(), axis.ticks.x = element_blank()), plot4 + theme(legend.position="none"), ncol = 1, sub = textGrob("Time (days)", gp = gpar(fontsize = 50, fontface = "bold"), vjust = 0.2)), legend, widths=unit.c(unit(4, "lines"), unit(1, "npc") - unit(4, "lines") - legend$width, legend$width), nrow=1) `,
                        q` print(main.plot) `,
                        q` dev.off() `,
    );
