#!/usr/bin/perl -w
# overlay plot of TOLUENE organic VOC intermediates reaction rates normalised by parent VOC total emissions, showing which intermediates are responsible for Ox production, for RACM and MCM 3.2
# Version 0: Jane Coates 12/02/2014
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, legend.title, plot.title, legend) { sums = colSums(data[,-1]); rep.no = nrow(data); regex = as.numeric(gsub("^[A-Z]+ + [A-Z]+", "", sums, perl = TRUE) ); type = c(); for ( i in 1:length(sums) ) { type = c(type, rep(regex[i], rep.no)) }; plot.data = melt(data = data, id = names(data)[1], measured = names(data)[-1] ); colnames(plot.data) = c("time", "reaction", "rate"); plot.data$subset.condition = type; reaction.levels = (levels(factor(plot.data$reaction))); plot.data$reaction = ordered(plot.data$reaction, levels = reaction.levels); plot.data = ddply( plot.data, .(reaction)); cols = colorRampPalette(brewer.pal(9, "Accent"))(nlevels(plot.data$reaction)); plot = ggplot(data = plot.data, aes(x = time, y = rate, fill = reaction)); plot = plot + geom_area(data = subset(plot.data, subset.condition > 0), position = "stack", alpha = 1); plot = plot + geom_area(data = subset(plot.data, subset.condition > 0), position = "stack", colour = "black", show_guide = FALSE); plot = plot + geom_area(data = subset(plot.data, subset.condition < 0), position = "stack", alpha = 1); plot = plot + geom_area(data = subset(plot.data, subset.condition < 0), position = "stack", colour = "black", show_guide = FALSE); plot = plot + guides(fill = guide_legend(title = legend.title)); plot = plot + ggtitle(plot.title); plot = plot + xlab("\nTime (days)\n"); plot = plot + scale_y_continuous(limits=c(-14, 10), breaks=seq(-14, 10, 2)); plot = plot + scale_x_continuous(limits=c(0, 7), breaks=seq(0, 7, 1)); plot = plot + theme_bw() ; plot = plot + theme(legend.key.size = unit(4.5, "cm")) ; plot = plot +  theme(axis.text.x = element_text(size = 120)) ; plot = plot + theme(axis.text.y = element_text(size = 120)) ; plot = plot + theme(axis.title.x = element_text(size = 120, face = "bold", vjust = 1, hjust = 0.47)) ; plot = plot + theme(legend.text = element_text(size = 90)) ; plot = plot + theme(legend.title = element_text(size = 110, face = "bold")) ; plot = plot + theme(legend.key = element_blank()) ; plot = plot + theme(axis.title.y = element_blank()) ; plot = plot + theme(plot.title = element_text(size = 130, face = "bold", vjust = 1)) ; plot = plot + theme(legend.justification = c(0.95, 0.05), legend.position = c(0.95, 0.05)) ; return(plot) } `);
     ...
     $R->run(q` racm.colours = c("Production Others" = "#696537", "ETHP + NO" = "#f9c600", "KETP + NO" = "#76afca", "NO + XO2" = "#dc3522", "ACO3 + NO" = "#8b1537", "HC3P + NO" = "#8c6238", "MO2 + NO" = "#e7e85e", "OH + ONIT" = "#9bb08f", "ADDC + O3" = "#0352cb", "CSL + NO3" = "#86b650", "ADDT + O3" = "#6c254f", "NO2 + PHO" = "#8ed6d5", "Consumption Others" = "#ee6738") `);
     $R->run(q` CairoPDF(file = "TOL_MCM_RACM_Ox_intermediates.pdf", width = 100, height = 70) `,
             q` y.label = textGrob(expression(bold(paste("Molecules (intermediate) ", s^-1, "/Molecules (VOC) x ", 10^5))), rot = 90, gp = gpar(fontsize = 120), vjust = 0.5)`,
             q` main.plot = grid.arrange(y.label, arrangeGrob(mcm3.2.plot + scale_fill_manual( name = "reaction", limits = mcm3.2.legend, values = mcm.colours) , racm.plot + theme(axis.text.y = element_blank()) + scale_fill_manual( name = "reaction", limits = racm.legend, values = racm.colours) , nrow = 1), nrow = 1, widths=unit.c(unit(12, "lines"), unit(1, "npc") - unit(12, "lines"))) `,
             q` print(main.plot) `,
             q` dev.off() `,
     );
