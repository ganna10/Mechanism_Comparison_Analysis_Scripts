...
# Version 1: Jane Coates 08/05/2014 changed plots to stacked bar charts
# Version 2: Jane Coates 03/06/2014 removing night-time production as it is insignificant compared to day-time production
# Version 3: Jane Coates 23/06/2014 XO2 included as its own category
# Version 4: Jane Coates 9/7/2014 removed XO2 and HO2 categories and attributed to C# of their reactants
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, plot.title) { data = ddply(data, .(time), colwise(sum)) ;
                                                        ...
                                                        plot = ggplot(data = plot.data, aes(x = time, y = rate, fill = C.number)); 
                                                        plot = plot + geom_bar(stat = "identity", width = 0.5) ;
                                                        plot = plot + ggtitle(plot.title); 
                                                        plot = plot + scale_y_continuous(limits = c(0, 35), breaks = seq(0, 35, 5)) ;
                                                        plot = plot + scale_fill_manual( limits = rev(C.number.levels), values = my.colours); 
                                                        plot = plot + theme_bw() ; 
                                                        plot = plot + theme(legend.key.size = unit(7.0, "cm")) ; 
    ...
    $R->run(q` CairoPDF(file = "toluene_carbon_breakdown.pdf", width = 130, height = 130) `,
