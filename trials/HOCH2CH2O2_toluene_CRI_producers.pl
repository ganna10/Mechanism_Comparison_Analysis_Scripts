#! /usr/bin/env perl
# HOCH2CH2O2 production contributions during Toluene degradation 
...
my $mecca = MECCA->new("$base/CRIv2_tagged/boxmodel");
...
my $eqnfile = "$base/CRIv2_tagged/gas.eqn";
...
$plot_data{"CRIv2"} = \@final_sorted_data;
...
$R->run(q` CairoPDF(file = "Toluene_HOCH2CH2O2_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
