#!/usr/bin/perl -w
# HC5 organic VOC intermediates reaction rates normalised by parent VOC total emissions, showing which intermediates are responsible for Ox production, for RADM2, RACM, RACM2 and MCM 3.2 - stacked bar plot
...
my $mcm_3_2_plot_title = "(a) Pentane - Organic Ox Budget using MCM v3.2";
...
my $radm2_plot_title = "(b) Pentane - Organic Ox Budget using RADM2";
...
my $racm_plot_title = "(c) Pentane - Organic Ox Budget using RACM";
...
my $racm2_plot_title = "(d) Pentane - Organic Ox Budget using RACM2";
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, legend.title, plot.title, legend) { data = ddply(data, .(time), colwise(sum)) ;
                                                                              ...
                                                                              plot = plot + scale_y_continuous(limits = c(-2, 24), breaks = seq(-2, 24, 5)) ;
     ...
     my $percent_file = "Percentage_of_total_HC5_organic_Ox_production.txt";
     ...
     #colours
     $R->run(q` mcm.colours = c("Production Others" = "#696537", "NO + OCTO2" = "#f9c600", "MEKCO2 + NO" = "#76afca", "HO3C86O2 + NO" = "#dc3522", "CO3C4CO3 + NO" = "#8c6238", "C84O2 + NO" = "#9bb08f", "CH3CO3 + NO" = "#8b1537", "CH3O2 + NO" = "#e7e85e", "C2H5O2 + NO" = "#0352cb", "C2H5CO3 + NO" = "#86b650", "Consumption Others" = "#ee6738") `,
             q` racm.colours = c("Production Others" = "#696537", "HC5P + NO" = "#f9c600", "KETP + NO" = "#76afca", "NO + XO2" = "#dc3522", "ACO3 + NO" = "#8b1537", "MO2 + NO" = "#e7e85e", "Consumption Others" = "#ee6738") `
     );

     #multiplot
     $R->run(q` CairoPDF(file = "HC5_Ox_intermediates_investigation.pdf", width = 130, height = 140) `,
             ...
             q` main.plot = grid.arrange(y.label, 
                                         arrangeGrob(mcm3.2.plot + theme(axis.text.x = element_blank()) + scale_fill_manual(name = "reaction", values = mcm.colours, limits = mcm3.2.legend), 
                                                     radm2.plot + theme(axis.text.x = element_blank(), axis.text.y = element_blank()) + scale_fill_manual(name = "reaction", values = racm.colours, limits = radm2.legend), 
