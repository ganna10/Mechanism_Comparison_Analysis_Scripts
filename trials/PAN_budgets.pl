#! /usr/bin/env perl
# PAN production and loss budgets in all mechanism
...
foreach my $mechanism (@mechanisms) {
    ...
    my $pan_file = "$base/${mechanism}_tagged/All_pans.txt";
    my $pans = read_file($pan_file);
    my @pans = map { split / / } @$pans;
    my @pan_producers = pan_producers($kpp, @pans);
    $families{"PANx"} = [ @pans ];
    #$families{"PANx"} = [ @pans, @pan_producers ];
    $kpp->family({
            name    => "PANx",
            members => $families{"PANx"},
            weights => $weights{"PANx"},
    }); 
    my (%production, %consumption);
    my $producers = $kpp->producing("PANx");
    my $producer_yields = $kpp->effect_on("PANx", $producers);
    my $consumers = $kpp->consuming("PANx");
    my $consumer_yields = $kpp->effect_on("PANx", $consumers);
...
foreach my $mechanism (sort keys %plot_data) {
    ...
    $R->set('filename', "${mechanism}_PAN_budget.csv");
...
$R->run(q` CairoPDF(file = "PAN_budgets.pdf", width = 8, height = 5.7) `,
        q` print(plot) `,
        q` dev.off() `,
);
