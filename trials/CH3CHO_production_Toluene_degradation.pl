#! /usr/bin/env perl
# CH3CHO production contributions during Toluene degradation in MCM v3.2 and MOZART
...
my @runs = qw( MCM_3.2_tagged MCM_3.1_tagged_3.2rates CRI_tagging RADM2_tagged RACM_tagging RACM2_tagged CBM4_tagging CB05_tagging );
my @mechanisms = ( "(a) MCMv3.2", "(b) MCMv3.1", "(c) CRIv2", "(d) RADM2", "(e) RACM", "(f) RACM2",  "(h) CBM-IV", "(i) CB05" );
my @base_name = qw( CH3CHO CH3CHO CH3CHO ALD ALD ACD ALD2 ALD2 );
my $index = 0;
...
$R->run(q` CairoPDF(file = "Toluene_CH3CHO_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
