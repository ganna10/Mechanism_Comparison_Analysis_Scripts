#! /usr/bin/env perl
# Compare PAR from alkane degradation (alkanes C >= 4) mixing ratio time series
...
my @pars = qw( PAR_NC4H10 PAR_IC4H12 PAR_NC5H12 PAR_IC5H12 PAR_NC6H14 PAR_NC7H16 PAR_NC8H18 );
my %mixing_ratio;
...
$R->run(q` CairoPDF(file = "PAR_mixing_ratios.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
