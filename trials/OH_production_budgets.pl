#! /usr/bin/env perl
# HOx production budgets in each mechanism
...
foreach my $mechanism (@mechanisms) {
    ...
    $families{"HOx_$mechanism"} = [ qw( OH HO2 HO2NO2 ) ];
...
$R->run(q` CairoPDF(file = "HOx_production_budgets_reactions.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    foreach my $item (keys %consumption) {
        ...
        $consumption{$item} = $integrate(0:13:2);
    }
    ...
    foreach my $item (keys %production) {
        ...
        my $integrate = $reshape->sumover;
        $production{$item} = $integrate(0:13:2);
    }
