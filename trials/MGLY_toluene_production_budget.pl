#! /usr/bin/env perl
# Plot MGLY production and consumption budgets for all mechanisms from toluene analysis
...
my @runs = qw( MCM_3.2_tagged MCM_3.1_tagged_3.2rates CRI_tagging MOZART_tagging RADM2_tagged RACM_tagging RACM2_tagged );
my @mechanisms = ( "(a) MCMv3.2", "(b) MCMv3.1", "(c) CRIv2", "(g) MOZART-4", "(d) RADM2", "(e) RACM", "(f) RACM2" );
my @base_name = qw( MGLYOX MGLYOX CARB6 CH3COCHO MGLY MGLY MGLY );
my @parent = qw( TOLUENE TOLUENE TOLUENE TOLUENE TOL TOL TOL );
...
$R->run(q` plotting = function (data, legend, mechanism) {  plot = ggplot(data, aes(x = Time, y = Rate, fill = Reaction)) ;
                                                            ...
                                                            plot = plot + theme_bw() ;
                                                            plot = plot + scale_y_continuous(limits = c(0, 3.5e7), breaks = seq(0, 3.5e7, 2.5e6), label = scientific_10) ;
...
$R->run(q` CairoPDF(file = "MGLY_toluene_production_comparison.pdf", width = 141, height = 200) `,
