...
my %families = (    'tagged_HO2x' => [ qw( HO2 HO2NO2 ) ],
                    'HO2x' => [ qw( HO2 HO2NO2 ) ] );
my (%weights, %species_production_rates, %production_reaction_rates) ;
...
sub plot { #create dataframe and then create plot
    ...
    $R->set('time', [@$time]);
    $R->run(q` plot.function = function(data, colours, names, plot.title, legend.title, y.max, y.breaks) {  data = ddply(data, .(time), colwise(sum)) ; 
                                                ...
                                                plot.data = ddply( plot.data, .(NMVOC)) ;
    ...
    $R->run(q` non.tagged.plot = plot.function(non.tagged.data, non.tagged.colours, non.tagged.names, non.tagged.title, non.tagged.legend, non.tagged.y.max, non.tagged.y.breaks) `);
     ...
     $R->run(q` CairoPDF(file = "MCM_Ox_tagged_non_tagged_production.pdf", width = 120, height = 60) `, 
