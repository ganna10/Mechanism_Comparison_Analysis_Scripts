#! /usr/bin/env perl
# HO2x production and loss budgets in CBs and MCM v3.2 during pentane degradation
...
foreach my $mechanism (@mechanisms) {
    ...
    for (0..$#$producers) {
        ...
        next unless (defined $parent and $parent eq "NC5H12");
...
$R->run(q` my.colours = c(  "Production Others" = "#696537", 
                            ...
                            "C2O3 + NO" = "#0e5c28", 
                            "MEO2 + NO" = "#2b9eb3" ) `); 
$R->run(q` data$Reaction = factor(data$Reaction, levels = c("Production Others", "MEO2 + NO", "C2O3 + NO", "ROR", "CO + OH", "HCHO + hv", "CH3O", "HCHO + OH")) `,
...
$R->run(q` CairoPDF(file = "Pentane_HO2x_budgets.pdf", width = 8, height = 5.7) `,
        q` print(plot) `,
        q` dev.off() `,
);
