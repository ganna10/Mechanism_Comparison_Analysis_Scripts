#! /usr/bin/env perl
# HCHO production contributions during CH4 degradation in MCM v3.2 and MOZART
...
$R->run(q` CairoPDF(file = "CH4_HCHO_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
