#! /usr/bin/env perl
# CH3CO3 budget during propane degradation in MCM, RADM2, RACM and RACM2
...
my @species = qw( CH3CO3 ACO3 ACO3 ACO3 );
my $index = 0;
...
$R->run(q` CairoPDF(file = "C3H8_CH3CO3_budget.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
