#! /usr/bin/env perl
# Compare reactions producing aldehydes in all mechanisms from NC5H12 degradation in CBs and MCM v3.2
...
$R->run(q` CairoPDF(file = "NC5H12_total_aldehyde_production_budget.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
