#! /usr/bin/env perl
# Calculate pentane and toluene OPE of each mechanism's model run base on Kleinman:2002 paper (P(Ox)/P(HNO3))
...
$R->run(q` CairoPDF(file = "C_number_distribution_OPE.pdf", width = 141, height = 200) `,
        q` print(plot) `,
        q` dev.off() `,
);
            ...
            for (0..$#$op_producers) { #get rates for all producing reactions
                ...
                foreach (@$reactants) {
                    if ($_ =~ /_/) {
                        ...
                        if (defined $carbons{$lookup}) {
                            print "$lookup and $carbons{$lookup}\n";
                            $production_rates{"C$carbons{$lookup}"} += $rate(1:$NTIME-2);
                        } else {
