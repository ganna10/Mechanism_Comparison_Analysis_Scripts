#! /usr/bin/env perl
# CH3CO3 production contributions during MEPROPENE degradation in MCM v3.2 and MOZART
...
my @species = qw( MEPROPENE BIGENE );
my $index = 0;
...
foreach my $mechanism (@mechanisms) {
    ...
    my $all_tagged_species = get_tagged_species("CH3CO3", $spcfile); 
    $families{$mechanisms[$index]} = [ @$all_tagged_species ];
    ($plot_data{$mechanisms[$index]}) = get_data($kpp, $mecca, $mechanism, $species[$index]);
    $index++;
}
...
$R->run(q` CairoPDF(file = "MEPROPENE_CH3CO3_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    for (0..$#$producers) { #get rates for all producing reactions
        ...
        next unless ($reaction =~ /MEPROPENE|BIGENE/);
        my $reaction_number = $kpp->reaction_number($reaction);
        my $rate = $producer_yields->[$_] * $mecca->rate($reaction_number); 
        $rate *= 0.65 if ($species eq "MOZART-4");
