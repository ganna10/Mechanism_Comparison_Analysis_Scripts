#! /usr/bin/env perl
# Calculate pentane OPE of each mechanism's model run base on Kleinman:2002 paper (P(Ox)/P(HNO3)) and express as contributing reactions
...
foreach my $run (@runs) {
    ...
    my $mech_species = get_model_name('Pentane', $run);
...
$R->run(q` my.colours = c(  "Production Others" = "#696537",
                            "NO + PEBO2" = "#603912", "NO + RN16O2" = "#603912", "HC5P + NO" = "#603912", "ALKO2 + NO" = "#603912",
                            ...
                            "NO + RN15AO2" = "#f8c56c",
                            "NO + RN13O2" = "#5b671f",
                            "HCHO + OH" = "#9bb08f", "CH2O + OH" = "#9bb08f",
                            "CH2O + hv" = "#0352cb", "HCHO + hv" = "#0352cb",
                            "CO + OH" = "#c9a415",
                            "MGLY + hv" = "#8ed6d2",
                            "HKET + OH" = "#035c28",
                            "KETP + NO" = "#c9a415",
...
$R->run(q` plotting = function (data) { plot = ggplot(data, aes(x = Time, y = OPE, fill = reaction)) ;
                                        ...
                                        plot = plot + scale_y_continuous(limits = c(0, 3e-9), breaks = seq(0, 3e-9, 1e-9), labels = scientific_10) ;
                                        plot = plot + theme_bw() ;
...
$R->run(q` CairoPDF(file = "pentane_OPE_comparison.pdf", width = 141, height = 200) `,
