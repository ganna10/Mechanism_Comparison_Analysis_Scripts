...
my @mechanisms = qw( MCMv3.2 MOZART-4 );
...
foreach my $mechanism (@mechanisms) {
    ...
    $data{$mechanism} = get_data($mecca, $kpp, $mechanism, $species[$index]);
    $index++;
}
...
sub get_data {
    ...
    foreach my $species (@loop) {
        ...
        for (0..$#$producers) {
            ...
            next unless ($reaction =~ /$VOC/);
            my $reaction_number = $kpp->reaction_number($reaction);
            my $rate = $mecca->rate($reaction_number) * $producer_yields->[$_];
            $rate *= 0.65 if ($VOC eq "BIGENE"); #ratio of MEPROPENE to BUTENE in BIGENE
            next if ($rate->sum == 0);
