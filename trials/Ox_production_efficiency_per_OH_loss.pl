...
foreach my $mechanism (@mechanisms) {
    ...
    $oh_loss_rate = $oh_loss_rate(0:13:2);
    
    my @VOCs = qw( Pentane Toluene );
    foreach my $NMVOC (@VOCs) {
        my $parent = get_mechanism_species($NMVOC, $mechanism);
        $data{$mechanism}{$NMVOC} = get_data($mecca, $kpp, $mechanism, $parent, $n_carbon{"Ox_$mechanism"}, $oh_loss_rate);
    }
}
...
$R->run(q` plot = ggplot(data, aes(x = Mechanism, y = OxPE, fill = C.Number)) `,
        ...
        q` plot = plot + ylab("Ox Production Efficiency per OH loss rate\n(molecules(Ox) / molecules(OH))") `,
...
$R->run(q` CairoPDF(file = "Ox_PE_per_OH_loss.pdf", width = 8.7, height = 10) `,
        q` print(plot) `,
        q` dev.off() `,
);

sub get_data {
    my ($mecca, $kpp, $mechanism, $VOC, $carbons, $oh_loss_rate) = @_;
    my %carbons = %$carbons;
    ...
    $production_rates{$_} /= -$oh_loss_rate foreach (keys %production_rates); #normalise by total OH loss rate
