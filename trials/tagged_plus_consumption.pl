#!/usr/bin/perl -w
# calculate Ox production from tagged model run and plot as stacked bar plot - plus consumption
...
my (%consumption_reaction_rates, %species_consumption_rates);
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` plot.data = melt( data = data, 
            ...
            #plot 
            q` CairoPDF(file = "tagged_plus_consumption.pdf", width = 70, height = 40) `,
