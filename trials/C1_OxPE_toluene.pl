#! /usr/bin/env perl
# Show reactions contributing to toluene C1 OxPE in each mechanism
...
$R->run(q` plotting = function (data) { plot = ggplot(data, aes(x = Time, y = Rate, fill = OxPE)) ;
                                        ...
                                        plot = plot + theme_bw() ;
                                        plot = plot + scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.1)) ;
...
$R->run(q` CairoPDF(file = "OxPEs_C1_toluene.pdf", width = 141, height = 200) `,
