#! /usr/bin/perl
# Analyse day-time radical budget during Toluene breakdown, compare for all mechanisms - stacked bar plot
# Version 0: Jane Coates 16/07/2014
...
sub get_category {
    ...
    if ($reactants =~ /hv/) {
        $category = 'Photolysis' ;
    } elsif ($reactants =~ /TOLUENE|TOL\b|TOL_TOLUENE\b/) {
        $category = 'VOC Initial Oxidation' ;
    } elsif ($reactants ~~ @$pans or $reactants eq "ONIT") {
        $category = 'PAN Family + Organic Nitrate Deposition';
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` mech.plot = function(data, plot.title, plot.legend) {data = ddply(data, .(time), colwise(sum)) ;
                                                                    ...
                                                                    plot = plot + theme(legend.position = "bottom", legend.title = element_blank(), legend.key.size = unit(7, "cm"), legend.key = element_blank()); 
                                                                    plot = plot + scale_y_continuous(limits = c(-60, 100), breaks = seq(-60, 100, 40)) ;
    ...
    $R->run(q` cb05.plot = mech.plot(cb05.data, cb05.plot.title, cb05.legend) `);
     ...
     $R->run(q` CairoPDF(file = "toluene_radical_budgets.pdf", width = 210, height = 210) `, 
