#!/usr/bin/perl -w
# calculate Ox budget from non-tagged model run and plot as stacked bar plot - including consumption
...
my (%production_reaction_rates, %consumption_reaction_rates);
...
foreach my $species (qw( Ox HO2x )) { #get all production and consumption rates
    ...
    for (0..$#$consumers) { #get rates for each consuming reactions
        my $reaction = $consumers->[$_];
		my $reaction_number = $kpp->reaction_number($reaction);
        my $rate = $consumer_yields->[$_] * $mecca->rate($reaction_number); 
        next if ($rate->sum == 0); # do not include reactions that do not occur
        my $string = $kpp->reaction_string($reaction);
        $consumption_reaction_rates{$species}{$string} += $rate(1:$ntime-2);
    }
} 
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(#reshape data frames
            ...
            #plot 
            q` CairoPDF(file = "non-tagged_plus_consumption.pdf", width = 70, height = 40) `,
