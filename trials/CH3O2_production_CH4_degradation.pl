#! /usr/bin/env perl
# CH3O2 production contributions during CH4 degradation in MCM v3.2 and MOZART
...
$R->run(q` CairoPDF(file = "CH4_CH3O2_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
