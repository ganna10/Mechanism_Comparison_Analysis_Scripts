#! /usr/bin/env perl
# Ketones during propane degradation in MCM and RADM2, RACM, RACM2
...
foreach my $mechanism (@mechanisms) {
    ...
    my $all_tagged_species = get_tagged_species($mechanism, $spcfile); 
...
$R->run(q` CairoPDF(file = "C3H8_Ketone_budget.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_tagged_species {
    ...
    if ($mechanism eq "RADM2" or $mechanism eq "RACM") {
        @tagged_species = qw( KET_HC3 );
    } elsif ($mechanism eq "RACM2") {
        @tagged_species = qw( DCB1_HC3 ACT_HC3 MEK_HC3 KET_HC3 );
    } else {
        @tagged_species = qw( CH3COCH3_C3H8 );
    }
