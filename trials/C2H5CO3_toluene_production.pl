#! /usr/bin/env perl
# C2H5CO3 production contributions during Toluene degradation in CRI and RACM2
...
my @runs = qw( CRI_tagging RACM2_tagged );
my @mechanisms = ( "CRIv2", "RACM2" );
my @base_name = qw( C2H5CO3 RCO3 );
my $index = 0;
...
$R->run(q` my.colours = c(  "C2H5O" = "#6c254f", "C2H5CO3 + NO" = "#6c254f", "RCO3 + NO" = "#6c254f",
                            "Production Others" = "#623812",
                            "HC3P + NO" = "#000000",
                            "NO + RCO3" = "#f9c500", "C2H5CO3 + NO" = "#f9c500",
...
$R->run(q` CairoPDF(file = "Toluene_C2H5CO3_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
