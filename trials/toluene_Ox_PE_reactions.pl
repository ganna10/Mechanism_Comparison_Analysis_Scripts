#! /usr/bin/env perl
# OxPE contributing reaction during Toluene degradation in each mechanism by normalising over total Ox consumption
...
$R->run(q` CairoPDF(file = "toluene_OxPE_by_reaction.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
