#!/usr/bin/perl
# H2O2 budget plot attributed back to the parent VOCs 
# Version 0 : Jane Coates 17/09/2014 
...
foreach my $run (@runs) {
    ...
    $families{$mechanisms[$array_index]} = [ qw(H2O2) ];
...
$R->run(q` plotting = function (data, legend, mechanism) {  cols = colorRampPalette(brewer.pal(8,"Dark2"))(nlevels(data$Reaction)) ; 
                                                            ...
                                                            plot = plot + geom_bar(data = subset(data, Rate < 0), stat = "identity") ;
                                                            plot = plot + scale_y_continuous(limits = c(-2e7, 2e7), breaks = seq(-2e7, 2e7, 7e6)) ;
                                                            plot = plot + theme_bw() ;
                                                            plot = plot + xlab("\n") ;
...
$R->run(q` CairoPDF(file = "H2O2_budget_assigned.pdf", width = 100, height = 100) `,
