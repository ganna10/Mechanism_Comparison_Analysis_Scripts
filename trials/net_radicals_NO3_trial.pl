...
foreach my $run (@runs) {
    ...
    $families{$mechanisms[$array_index]} = [ "HO2NO2", @radicals, @no2_reservoirs, "NO3", "N2O5" ] ;
...
$R->run(q` plotting = function (data, legend, mechanism) {  cols = colorRampPalette(brewer.pal(8,"Dark2"))(nlevels(data$Reaction)) ; 
                                                            ...
                                                            plot = plot + scale_y_continuous(limits = c(-1.1e9, 1.1e9), breaks = seq(-1.1e9, 1.1e9, 1e8)) ;
...
$R->run(q` CairoPDF(file = "MCM_all_radicals_NO3_budget_assigned.pdf", width = 100, height = 100) `,
