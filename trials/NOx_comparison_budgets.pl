...
foreach my $run (@runs) {
    ...
    $families{$mechanisms[$array_index]} = [ qw( NO NO2 NO3 N2O5 ) ];
    $weights{$mechanisms[$array_index]} = { N2O5 => 2 } ;
...
$R->run(q` plotting = function (data, legend, mechanism) {  plot = ggplot(data, aes(x = time, y = Rate, fill = Reaction)) ;
                                                            ...
                                                            plot = plot + geom_bar(data = subset(data, Rate < 0), stat = "identity") ;
                                                            plot = plot + scale_y_continuous(limits = c(-2e9, 2e9), breaks = seq(-2e9, 2e9, 5e8)) ;
                                                            plot = plot + theme_bw() ;
...
$R->run(q` CairoPDF(file = "NOx_budget_assigned.pdf", width = 141, height = 200) `,
