#! /usr/bin/env perl
# PAR production budget in IC5H12 degradation, as this has highest emissions of alkanes with C >= 4
...
foreach my $mechanism (@mechanisms) {
    ...
    my $reaction = $kpp->reacting_with("OH", "PAR_IC5H12");
...
$R->run(q` CairoPDF(file = "PAR_reaction_Rates.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
