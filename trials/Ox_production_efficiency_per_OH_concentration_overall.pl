#! /usr/bin/env perl
# look at Ox production efficiency per OH concentration for whole model run
...
$R->run(q` CairoPDF(file = "Ox_PE_overall_per_OH_conc.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    $oh_conc = $oh_conc->reshape($N_PER_DAY, $N_DAYS);
    $oh_conc = $oh_conc->sumover;
    ...
    $production_rates /= $oh_conc; #normalising by OH concentration
