#! /usr/bin/env perl
# Compare reactions producing aldehydes in all mechanisms from TOLUENE degradation in CBs and MCM v3.2
...
$R->run(q` CairoPDF(file = "TOLUENE_total_aldehyde_production_budget.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
