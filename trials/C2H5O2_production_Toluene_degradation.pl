#! /usr/bin/env perl
# C2H5O2 production contributions during Toluene degradation in CRI and RACM2
...
$R->run(q` CairoPDF(file = "Toluene_C2H5O2_production_contributors.pdf") `,
        q` print(plot) `,
        q` dev.off() `,
);
