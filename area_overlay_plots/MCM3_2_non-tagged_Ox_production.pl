#!/usr/bin/perl -w
# calculate Ox production from non-tagged model run and plot as overlay
# Version 0: Jane Coates 12/08/2014
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` plot.data = melt( data = data, 
            ...
            #plot 
            q` CairoPDF(file = "MCMv3_2_Ox_non_tagged_budget_overlay.pdf", width = 50, height = 40) `,
