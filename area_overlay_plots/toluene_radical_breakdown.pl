#! /usr/bin/perl
# Analyse radical budget during toluene breakdown, compare for all mechanisms
...
($production_rates{'radicals_mozart'}, $consumption_rates{'radicals_mozart'}) = get_rates($species[0], 'radicals_mozart', $mozart_kpp, $mozart_mecca);
...
($production_rates{'radicals_radm2'}, $consumption_rates{'radicals_radm2'}) = get_rates($species[1], 'radicals_radm2', $radm2_kpp, $radm2_mecca);
...
($production_rates{'radicals_racm'}, $consumption_rates{'radicals_racm'}) = get_rates($species[1], 'radicals_racm', $racm_kpp, $racm_mecca);
...
($production_rates{'radicals_racm2'}, $consumption_rates{'radicals_racm2'}) = get_rates($species[1], 'radicals_racm2', $racm2_kpp, $racm2_mecca);
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` CairoPDF(file = "toluene_radical_budgets.pdf", width = 190, height = 200) `, 
