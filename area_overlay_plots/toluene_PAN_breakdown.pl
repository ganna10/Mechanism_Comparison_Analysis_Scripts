#! /usr/bin/perl
# Analyse PAN budget during Toluene breakdown, compare for all mechanisms
...
($production_rates{'PAN_mozart'}, $consumption_rates{'PAN_mozart'}) = get_rates($species[0], 'PAN_mozart', $mozart_kpp, $mozart_mecca);
...
($production_rates{'PAN_radm2'}, $consumption_rates{'PAN_radm2'}) = get_rates($species[1], 'PAN_radm2', $radm2_kpp, $radm2_mecca);
...
($production_rates{'PAN_racm'}, $consumption_rates{'PAN_racm'}) = get_rates($species[1], 'PAN_racm', $racm_kpp, $racm_mecca);
...
($production_rates{'PAN_racm2'}, $consumption_rates{'PAN_racm2'}) = get_rates($species[1], 'PAN_racm2', $racm2_kpp, $racm2_mecca);
...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` CairoPDF(file = "toluene_PAN_budgets.pdf", width = 190, height = 200) `, 
