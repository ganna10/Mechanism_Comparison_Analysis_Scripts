#! /usr/bin/env perl
# Allocation HCHO net production to VOC sources in each mechanism
...
$R->run(q` plot = ggplot(data, aes(x = Time, y = Rate, fill = VOC)) `,
        ...
        q` plot = plot + xlab("\n") `,
        q` plot = plot + ylab(expression(bold(paste("Formaldehyde Production Rates Allocated to Parent VOC (molecules (HCHO) ", cm^-3, s^-1,")")))) `,
