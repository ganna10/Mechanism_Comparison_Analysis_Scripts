...
foreach my $mechanism (sort keys %data) {
    ...
    $R->set('loss.rate', [ map { $_ } $data{$mechanism}->dog ]);
    $R->run(q` data[mechanism] = loss.rate `);
}
$R->run(q` data = gather(data, Mechanism, Loss.Rate, -Time) `);
...
sub get_data {
    ...
    $overall_carbon_loss_rate = $overall_carbon_loss_rate->copy->reshape($N_PER_DAY, $N_DAYS);
