...
sub plot { #create dataframe and then create plot
    ...
    $R->run(q` data = ddply(data, .(time), colwise(sum)) `,
            ...
            q` my.colours = c( "Production Others" = "#696537", "C2H5O = CH3CHO + HO2" = "#f9c600", "C2H5O2 + NO = C2H5O + NO2" = "#76afca", "HCHO + hv = CO + HO2 + HO2" = "#dc3522", "CH3CO3 + NO = CH3O2 + NO2" = "#8c6238", "HCHO + OH = CO + HO2" = "#9bb08f", "CH3O = HCHO + HO2" = "#8b1537", "CH3O2 + NO = CH3O + NO2" = "#e7e85e", "CO + OH = HO2" = "#2c9def", "O3 + OH = HO2" = "#603912", "IC3H7O2 + NO = IC3H7O + NO2" = "#b569b3" ) `,
            q` my.names = c( "Production Others" = "Production Others", "C2H5O = CH3CHO + HO2" = "C2H5O = CH3CHO + HO2", "C2H5O2 + NO = C2H5O + NO2" = "C2H5O2 + NO = C2H5O + NO2", "HCHO + hv = CO + HO2 + HO2" = "HCHO + hv = CO + HO2 + HO2", "CH3CO3 + NO = CH3O2 + NO2" = "CH3CO3 + NO = CH3O2 + NO2", "HCHO + OH = CO + HO2" = "HCHO + OH = CO + HO2", "CH3O = HCHO + HO2" = "CH3O = HCHO + HO2", "CH3O2 + NO = CH3O + NO2" = "CH3O2 + NO = CH3O + NO2", "CO + OH = HO2" = "CO + OH = HO2" ) `,
            
            q` scientific_10 <- function(x) { parse(text=gsub("e", " %*% 10^", scientific_format()(x))) } `, #scientific label format for y-axis

            #plot 
            q` CairoPDF(file = "MCMv3_2_Ox_no_tagging_budget_overlay.pdf", width = 28, height = 40) `,
            q` plot = ggplot(plot.data, aes(x = time, y = rate, fill = NMVOC)) `,
            q` plot = plot + geom_bar(stat = "identity", width = 0.6) `,
            q` plot = plot + theme_bw() `,
            q` plot = plot + ggtitle("(a) No Tagging") `,
            ...
            q` plot = plot + scale_fill_manual( name = "Without Tagging",
