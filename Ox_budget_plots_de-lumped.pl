...
$R->run(q` print.mechanisms = c("RADM2", "MCMv3.2") `);
$R->run(q` print.data = filter(data, Mechanism %in% print.mechanisms) `);
my $p = $R->run(q` print(print.data) `);
print $p, "\n";
...
sub get_data {
    ...
    if ($mechanism eq "RADM2" or $mechanism eq "RACM") {
        ...
        $production{"Ox_$mechanism"}{"C3H6"} = 0.883 * $production{"Ox_$mechanism"}{"OLT"}; 
        $production{"Ox_$mechanism"}{"BUT1ENE"} = 0.117 * $production{"Ox_$mechanism"}{"OLT"}; 
        ...
        $production{"Ox_$mechanism"}{"BUT1ENE"} = 0.117 * $production{"Ox_$mechanism"}{"OLT"}; 
        delete $production{"Ox_$mechanism"}{"OLT"};
        $production{"Ox_$mechanism"}{"TOLUENE"} = 0.868 * $production{"Ox_$mechanism"}{"TOL"};
        $production{"Ox_$mechanism"}{"EBENZ"} = 0.132 * $production{"Ox_$mechanism"}{"TOL"};
        delete $production{"Ox_$mechanism"}{"TOL"};
    } elsif ($mechanism =~ /MOZ/) {
