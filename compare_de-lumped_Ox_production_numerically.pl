#!/usr/bin/env perl 
# Compare first day Ox production of all VOC in each mechanism, lumped species are de-lumped into constituent VOC
...
sub get_data {
    ...
    foreach my $process (keys %{$production{"Ox_$mechanism"}}) {
        if ($process =~ /=/) {
            ...
            $production{"Ox_$mechanism"}{$process} = $integrate->at(0);
        }
    }
    ...
    foreach my $VOC (sort keys %{$production{"Ox_$mechanism"}}) {
        ...
        $production{"Ox_$mechanism"}{$name} = $production{"Ox_$mechanism"}{$VOC};
        delete $production{"Ox_$mechanism"}{$VOC};
    }
